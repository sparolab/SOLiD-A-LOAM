...
// std::queue<std::pair<int, int> > solidLoopICPBuf;
std::queue<std::tuple<int, int, float>> solidLoopICPBuf; // for ICP initial
...
std::string pgKITTIformat, pgScansDirectory, pgSOLiDsDirectory;
...
void saveSOLiD(std::string fileName, Eigen::VectorXd vector, std::string delimiter = " ")
{
    // delimiter: ", " or " " etc.
...
void runISAM2opt(void)
{
    ...
    isam->update();
    std::cout << "isam size : " << isam->size() << std::endl;
...
/***************************************** Without Initial ****************************************/
std::optional<gtsam::Pose3> doICPVirtualRelativeWithoutInitial( int _loop_kf_idx, int _curr_kf_idx, float _yaw_diff)
{
    // parse pointclouds
    ...
    float icpScoreThreshold = 0.3; // user parameter but fixed low value is safe. 
    if (icp.hasConverged() == false || icp.getFitnessScore() > icpScoreThreshold) {
        std::cout << "\033[1;31m[SOLiD loop] ICP fitness test failed (" << icp.getFitnessScore() << " > " << icpScoreThreshold << "). Reject this SOLiD loop.\033[0m" << std::endl;
        failedICP_cnt++;
        // std::cout << "If ICP failed without initial  " << failedICP_cnt << std::endl;
        return std::nullopt;
    } else {
        std::cout << "\033[1;32m[SOLiD loop] ICP fitness test passed (" << icp.getFitnessScore() << " < " << icpScoreThreshold << "). Add this SOLiD loop.\033[0m" << std::endl;
    }
...
} // doICPVirtualRelativeWithoutInitial
/***********************************************************************************************/
...
std::optional<gtsam::Pose3> doICPVirtualRelativeWithInitial( int _loop_kf_idx, int _curr_kf_idx, float _yaw_diff)
{
    ...
    pcl::transformPointCloud(*cureKeyframeCloud, *transformed_curr, rotation_matrix);    
    ...
    if (icp.hasConverged() == false || icp.getFitnessScore() > icpScoreThreshold) {
        std::cout << "\033[1;31m[SOLiD loop] ICP fitness test failed (" << icp.getFitnessScore() << " > " << icpScoreThreshold << "). Reject this SOLiD loop.\033[0m" << std::endl;
        failedICP_cnt++;
        std::cout << "If ICP failed with initial  " << failedICP_cnt << std::endl;
        return std::nullopt;
    } else {
        std::cout << "\033[1;32m[SOLiD loop] ICP fitness test passed (" << icp.getFitnessScore() << " < " << icpScoreThreshold << "). Add this SOLiD loop.\033[0m" << std::endl;
    }
...
void process_pg()
{
    while(1)
    {
		while ( !odometryBuf.empty() && !fullResBuf.empty() )
        {
            ...
            solidModule.down_sampling(*thisKeyFrame, thisKeyFrameDS);
            ...
            solidModule.makeAndSaveSolid(*thisKeyFrameDS);
            ...
            // const auto& curr_solid = solidModule.getConstRefRecentSOLiD();
            // saveSOLiD(pgSOLiDsDirectory + curr_node_idx_str + ".solid", curr_solid);
...
void performSOLiDLoopClosure(void)
{
    ...
    int SOLiDclosestHistoryFrameID = detectResult.first;
    if( SOLiDclosestHistoryFrameID != -1 ) { 
        ...
        std::cout << "Loop detected! - between " << prev_node_idx << " and " << curr_node_idx << "" << std::endl;
        std::cout << "Their angle difference is " << yaw_diff << " [rad]  or  " << rad2deg(yaw_diff) << " [deg]" << std::endl; 

        mBuf.lock();
        // solidLoopICPBuf.push(std::pair<int, int>(prev_node_idx, curr_node_idx));
        // Deliver prev_, curr_, and angle difference[rad] for ICP
        solidLoopICPBuf.push(std::make_tuple(prev_node_idx, curr_node_idx, yaw_diff));
...
void process_icp(void)
{
    while(1)
    {
		while ( !solidLoopICPBuf.empty() )
        {
            if( solidLoopICPBuf.size() > 30 ) {
            ...
            mBuf.lock();
            auto loop_info = solidLoopICPBuf.front();
            solidLoopICPBuf.pop();
            mBuf.unlock();

            const int prev_node_idx = std::get<0>(loop_info);
            const int curr_node_idx = std::get<1>(loop_info);
            const float yaw_diff = std::get<2>(loop_info); // yaw 차이 값 추출
            std::optional<gtsam::Pose3> relative_pose_optional;
            if (is_withInitialICP)
            {
                std::cout << "\033[1;34mRunning ICP with initial guess...\033[0m" << std::endl;
                std::cout << "If ICP failed with initial  " << failedICP_cnt << std::endl;
                relative_pose_optional = doICPVirtualRelativeWithInitial(prev_node_idx, curr_node_idx, yaw_diff);
            }                
            else
            {
                std::cout << "\033[1;34mRunning ICP without initial guess...\033[0m" << std::endl;
                std::cout << "If ICP failed without initial  " << failedICP_cnt << std::endl;
                relative_pose_optional = doICPVirtualRelativeWithoutInitial(prev_node_idx, curr_node_idx, yaw_diff);
            }
            ICP_cnt++;            
            std::cout << "Total ICP counts               " << ICP_cnt << std::endl;
...
void pubMap(void)
{
    ...
    pcl::PointCloud<PointType>::Ptr lastTransformedCloud(new pcl::PointCloud<PointType>());
    for (int node_idx=0; node_idx < recentIdxUpdated; node_idx++) {
        ...
        if (node_idx == recentIdxUpdated - 1) {
            *lastTransformedCloud = *local2global(keyframeLaserClouds[node_idx], keyframePosesUpdated[node_idx]);
        }
    }
    mKF.unlock(); 
    ...
    pcl::toROSMsg(*lastTransformedCloud, globalCloudMsg);
    globalCloudMsg.header.frame_id = "camera_init";
    pubGlobalCloud.publish(globalCloudMsg);
...
void saveGlobalMap()
{
    if(laserCloudMapPGO->size() > 0)
    {
        std::cout << "Saving global map to PCD file..." << std::endl;
        std::string globalMapFilename = save_directory + "globalMap.pcd";
        pcl::io::savePCDFileBinary(globalMapFilename, *laserCloudMapPGO);
        std::cout << "Global map saved to " << globalMapFilename << std::endl;
    }
}
...
int main(int argc, char **argv)
{
    ...
    pgSOLiDsDirectory = save_directory + "SOLiDs/"; 
    unused = system((std::string("exec rm -r ") + pgSOLiDsDirectory).c_str());
    unused = system((std::string("mkdir -p ") + pgSOLiDsDirectory).c_str());

    // system params 
	nh.param<double>("keyframe_meter_gap", keyframeMeterGap, 0.0); // pose assignment every k m move 
	nh.param<double>("keyframe_deg_gap", keyframeDegGap, 0.0); // pose assignment every k deg rot 
    keyframeRadGap = deg2rad(keyframeDegGap);

	// SOLiD params
    nh.param<double>("r_solid_thres", R_SOLiD_THRES, 0.99);
    nh.param<double>("fov_u", FOV_u, 2);
    nh.param<double>("fov_d", FOV_d, -24.8);
    nh.param<int>("num_angle", NUM_ANGLE, 60);
    nh.param<int>("num_range", NUM_RANGE, 40);
    nh.param<int>("num_height", NUM_HEIGHT, 32);
    nh.param<int>("min_distance", MIN_DISTANCE, 3);
    nh.param<int>("max_distance", MAX_DISTANCE, 80);
    nh.param<double>("voxel_size", VOXEL_SIZE, 0.4);
    nh.param<int>("num_exclude_recent", NUM_EXCLUDE_RECENT, 30);
    nh.param<int>("num_candidates_from_tree", NUM_CANDIDATES_FROM_TREE, 3);
    ...
    // ICP param
    nh.param<float>("icpScoreThreshold", icpScoreThreshold, 0.3);
    ...
    pubGlobalCloud = nh.advertise<sensor_msgs::PointCloud2>("/aft_global_cloud", 100);
